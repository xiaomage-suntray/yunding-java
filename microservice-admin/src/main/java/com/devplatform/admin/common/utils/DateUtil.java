...
public class DateUtil {
	...
	static SimpleDateFormat sdfLong = new SimpleDateFormat(
			"yyyy-MM-dd");
	static SimpleDateFormat sdfLong2 = new SimpleDateFormat(
			"yyyy/MM/dd");
	 ...
	 * @return
	 */
	private static List<Date> getWorkDates(int year,int month){    
	 ...
	 * @return
	 */
	public static List<Date> daysBetweenExcludeWeekendsDates(Date newDate, Date oldDate) {
		 List<Date> dates = new ArrayList<Date>();    
	        ...
	        while(!calo.after(caln)){    
	            int day = calo.get(Calendar.DAY_OF_WEEK);    
	                
	            if(!(day == Calendar.SUNDAY || day == Calendar.SATURDAY)){    
	                dates.add((Date)calo.getTime().clone());    
	            }    
	            calo.add(Calendar.DATE, 1);    
	        }    
	        return dates;    
	}
...
//		strFormatTo = strFormatTo.replace('/', '-');
     ...
     * @return Dates List
     */
    public static List<Date> getDatesBetween2Days(Date start, Date end) {
     ...
     * @return Dates List
     */
    public static List<Date> getDatesBetween2DaysIncludeStartAndEnd(Date start, Date end) {
        List<Date> result = new ArrayList<Date>();
        result.add(start);
        Calendar temp = Calendar.getInstance();
        temp.setTime(start);
     ...
     * @return Dates List
     */
    public static List<Date> getDatesAdd4DayFromStartDate(Date start) {
        List<Date> result = new ArrayList<Date>();
        result.add(start);
        Calendar temp = Calendar.getInstance();
        temp.setTime(start);
     ...
     * @return
     */
    public static boolean isNextMonth(Date date1, Date date2) {
        Calendar cal1 = Calendar.getInstance();
        cal1.setTime(date1);
     ...
     * @return
     */
    public static void set2359ofDay(Date day) {
    	day.setHours(23);
    	day.setMinutes(59);
    	day.setSeconds(59);
    }
    ...
    /**
     * 获得指定月份的日期格式 yyyyMM ,addMonth 是加减月份
     * @param date1
     * @param addMonth
     * @return
     */
    public static String getMonthString(Date date1, int addMonth) {
    	 Calendar cal = Calendar.getInstance();
    	 cal.setTime(date1);
    	 cal.add(Calendar.MONTH, addMonth);
    	 SimpleDateFormat sdfShort = new SimpleDateFormat(
    				"yyyyMM");
    	 return sdfShort.format(cal.getTime());
    }
     ...
     * @return
     */
    public static String getOnlyMonthString(Date date1, int addMonth) {
    	 Calendar cal = Calendar.getInstance();
    	 cal.setTime(date1);
    	 cal.add(Calendar.MONTH, addMonth);
    	 SimpleDateFormat sdfShort = new SimpleDateFormat(
    ...
    /**
     * 判断指定日期是否在同一个月中，用于检查该日期是否跨月
     * @param start
     * @param end
     * @return
     */
    public static boolean isAcrossMonth(Date start,Date end) {
    ...
    public static Date getFirstDayOfMonth(Date date,Integer addMonth) {
    	Calendar c = Calendar.getInstance();    
        c.setTime(date);
        c.set(Calendar.HOUR_OF_DAY, 0); 
        c.set(Calendar.SECOND, 0); 
	...
	public static void main(String[] args) {
		try {
			System.out.println(getAllWorkDaysInMonth(new Date()));
			System.out.println(String.format("My name is %1$s.","zhangsan",23)); 
			String s="27446.956521739132";
			String ss=String.format("%.2f", Double.valueOf(s));
		 
			System.out.println(ss);
//			
//			System.out.println(isSameDate(new Date(),new Date("2015/12/16")));
//			System.out.println(DateUtil.toDayToStr("yyyy年MM月dd日"));
//			System.out.println("###"+daysBetweenExcludeWeekendsDates(new Date("2015/12/31"),new Date("2015/12/01")));
...
//					+ getDateFromString("2009-11-18 19:14:31"));
	...
	public static int getWeekNumFromDay(Date dt){
		Calendar cal=Calendar.getInstance(); 
		cal.setTime(dt);
	...
	public static List<Date> getAllWorkDaysInMonth(Date startDate) {
		 //获取当前月第一天：
        Calendar c = Calendar.getInstance();    
        c.setTime(startDate);
        c.set(Calendar.DAY_OF_MONTH,1);//设置为1号,当前日期既为本月第一天 
        ...
        return daysBetweenExcludeWeekendsDates(ca.getTime(),c.getTime());
	}
	
	//yyyyMMddHHmmssSSS+6位随机数
	public static String getRandomFileName(Integer type) {
		String str = new SimpleDateFormat("yyyyMMddhhmmssSSS").format(new Date());
		int rannum = 0;
		if (type == 6) {
			 rannum = (int)((Math.random()*9+1)*100000);// 获取6位随机数
		} else if (type == 3) {
